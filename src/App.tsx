...
import { 
  LicenseInfo, 
  getLicenseInfo, 
  updateAndCheckMonotonicTime 
} from './lib/offlineLicense';
import { 
  saveOrderToIndexedDB, 
  getPendingSyncItems, 
  markItemSynced 
...
export default function App() {
  ...
  // --- Offline License & Anti-Tampering Grace Period State ---
  ...
  useEffect(() => {
    refreshLicenseState();
    const interval = setInterval(refreshLicenseState, 20000);
  ...
  const completeSale = (posApprovalCode?: string) => {
    ...
    // Save to IndexedDB asynchronously for robust offline persistence & sync queue
    saveOrderToIndexedDB(newOrder, typeof navigator !== 'undefined' ? !navigator.onLine : false);

    // Update monotonic anti-tampering time watermark
    updateAndCheckMonotonicTime();
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col select-none" style={{ direction: isRtl ? 'rtl' : 'ltr' }} id="pos-root">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0 print:hidden shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <OfflineIndicator onTriggerSync={async () => {
            try {
              const items = await getPendingSyncItems();
              for (const item of items) {
                if (item.syncId) {
                  await markItemSynced(item.syncId);
                }
              }
              addToast('تمت مزامنة العمليات المحلية بنجاح وحفظها في قاعدة البيانات!', 'success');
            } catch (err: any) {
              addToast('خطأ أثناء المزامنة: ' + (err?.message || 'غير معروف'), 'error');
            }
          }} />
          ...
          {/* Annual Offline License Status Pill */}
          {licenseInfo && (
            <button
              ...
              title="رخصة نظام قيد السنوية (500 ريال) - انقر لعرض التفاصيل ومعرّف الجهاز"
            >
              {licenseInfo.status === 'active' ? (
                <>
                  <ShieldCheck className="w-4 h-4 text-emerald-600" />
                  <span>اشتراك سنوي نشط ({licenseInfo.daysRemaining} يوم)</span>
                </>
              ) : licenseInfo.status === 'grace_period' ? (
                <>
                  <AlertTriangle className="w-4 h-4 text-amber-600" />
                  <span>مهلة السداد (بقي {licenseInfo.graceDaysRemaining} يوم)</span>
                </>
              ) : (
                <>
                  <Lock className="w-4 h-4 text-rose-600" />
                  <span>تفعيل الاشتراك (500 ر.س)</span>
                </>
              )}
            </button>
          )}
          ...
          {/* Quick Theme Switcher: White & Blue (Light) vs Matte Black & White (Dark) */}
          <div className="flex items-center p-0.5 bg-slate-100 dark:bg-zinc-800/90 border border-slate-200 dark:border-zinc-700 rounded-xl shadow-xs" id="header-theme-switcher">
            <button
              onClick={() => {
                setThemeMode('light');
                setPrimaryColor('#2563eb');
                addToast('تم تفعيل شاشة أبيض وأزرق ملكي (الوضع الفاتح) ☀️', 'info');
              }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                themeMode === 'light'
                  ? 'bg-white text-blue-600 shadow-xs border border-blue-200/80 ring-1 ring-blue-500/20'
                  : 'text-slate-500 hover:text-slate-800 dark:text-zinc-400 dark:hover:text-white'
              }`}
              id="btn-theme-white-blue"
              title="تفعيل الشاشة البيضاء والزرقاء (فاتح ☀️)"
            >
              <div className="w-2.5 h-2.5 rounded-full bg-blue-600 border border-white shadow-xs"></div>
              <span>أبيض وأزرق ☀️</span>
            </button>
            ...
            <button
              onClick={() => {
                ...
                addToast('تم تفعيل شاشة أسود مطفي وأبيض (الوضع الداكن) 🌙', 'info');
              }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                themeMode === 'dark'
                  ? 'bg-zinc-900 text-white shadow-xs border border-zinc-600 ring-1 ring-white/20'
                  : 'text-slate-500 hover:text-slate-800 dark:text-zinc-400 dark:hover:text-white'
              }`}
              id="btn-theme-matte-black"
              title="تفعيل شاشة أسود مطفي وأبيض (داكن 🌙)"
            >
              <div className="w-2.5 h-2.5 rounded-full bg-zinc-950 border border-zinc-400 shadow-xs"></div>
              <span>أسود مطفي 🌙</span>
            </button>
          </div>
      ...
      {/* 1.4 Offline License Grace Period Alert Banner (30-day grace period) */}
      {licenseInfo && licenseInfo.status === 'grace_period' && (
        <div
          id="qayd-license-grace-banner"
          className="bg-gradient-to-r from-amber-600 via-amber-700 to-orange-700 text-white px-4 py-2.5 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs font-bold shrink-0 print:hidden shadow-md"
        >
          <div className="flex items-center gap-2 text-right">
            <AlertTriangle className="w-5 h-5 text-amber-200 shrink-0 animate-bounce" />
            <span>
              تنبيه الاشتراك السنوي لنظام قيد: أنت تعمل حالياً ضمن مهلة السداد (30 يوماً). متبقي <strong>{licenseInfo.graceDaysRemaining} يوماً</strong> قبل إيقاف وقفل النظام تلقائياً. قيمة الاشتراك السنوي 500 ريال.
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => setIsLicenseModalOpen(true)}
              className="px-3.5 py-1.5 bg-white text-slate-900 hover:bg-amber-50 rounded-xl text-xs font-black shadow-sm transition active:scale-95 cursor-pointer flex items-center gap-1.5"
            >
              <KeyRound className="w-3.5 h-3.5 text-amber-600" />
              <span>تفعيل / تجديد الاشتراك (500 ر.س)</span>
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 relative text-right flex flex-col gap-5">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-4 text-right">
              ...
              {/* 1. One-Click Theme Profiles (White & Blue vs Matte Black & White) */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 block">
                  أنماط الشاشة الجاهزة بنقرة واحدة (One-Click Theme Profiles):
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  {THEME_PROFILES.map((profile) => {
                    const isProfileActive = 
                      themeMode === profile.mode && 
                      primaryColor.toLowerCase() === profile.primaryColor.toLowerCase();
                    return (
                      <button
                        key={profile.id}
                        type="button"
                        onClick={() => {
                          setThemeMode(profile.mode);
                          setPrimaryColor(profile.primaryColor);
                          addToast(`تم تفعيل نمط ${profile.nameAr} بنجاح!`, 'success');
                        }}
                        className={`text-right p-3 rounded-2xl border transition-all cursor-pointer relative flex flex-col justify-between group ${
                          isProfileActive
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full border border-slate-200 bg-slate-50 text-slate-700">
                            {profile.badge}
                          </span>
                          {isProfileActive && (
                        ...
                        {/* Visual Miniature Screen Representation */}
                        <div
                          className="h-11 rounded-xl border border-slate-200 p-1.5 flex items-center justify-between mb-2.5 shadow-inner"
                          style={{ backgroundColor: profile.previewBg }}
                        >
                          <div
                            className="w-1/3 h-full rounded-md border border-slate-400/20 flex items-center justify-center"
                            style={{ backgroundColor: profile.previewCard }}
                          >
                            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: profile.previewAccent }}></span>
                          </div>
                          <div
                            className="w-1/2 h-6 rounded-md flex items-center justify-center text-[9px] font-extrabold shadow-xs"
                            style={{
                              backgroundColor: profile.previewAccent,
                              color: profile.previewAccent === '#ffffff' ? '#121212' : '#ffffff'
                            }}
                          >
                            زر الكاشير
                          </div>
                        ...
                        <div>
                          <h5 className="text-xs font-bold text-slate-800">{profile.nameAr}</h5>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-snug">{profile.descriptionAr}</p>
                        </div>
                      </button>
                    );
                  })}
              ...
              {/* 2. Manual Light vs Dark Mode Switcher */}
              <div className="space-y-1.5 pt-1">
                <label className="text-xs font-bold text-slate-700 block">تبديل الإضاءة يدويًا (Light / Dark Mode):</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                      themeMode === 'light'
                        ? 'border-blue-600 bg-white text-blue-700 shadow-sm ring-2 ring-blue-500/20'
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                      themeMode === 'dark'
                        ? 'border-zinc-700 bg-zinc-900 text-white shadow-sm ring-2 ring-white/20'
                        : 'border-slate-200 bg-slate-100/70 text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    <Moon className={`w-4 h-4 ${themeMode === 'dark' ? 'text-indigo-400' : 'text-slate-400'}`} />
                    <span>الوضع الداكن (أسود مطفي) 🌙</span>
                  </button>
                </div>
              </div>
            ...
            {/* Offline PWA & Annual Subscription System Section */}
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 text-right">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-slate-800">
                      الاشتراك السنوي ونظام القفل والتفعيل أوفلاين (QAYD Lock)
                    </h3>
                    <p className="text-[10px] text-slate-500">
                      ترخيص سنوي بقيمة 500 ريال سعودي بمهلة سداد 30 يوماً وتشفير رقمي مستقل بدون إنترنت
                    </p>
                  </div>
                </div>
                {licenseInfo && (
                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black ${
                    ...
                    {licenseInfo.status === 'active' ? `نشط (${licenseInfo.daysRemaining} يوم متبقي)` : licenseInfo.status === 'grace_period' ? `مهلة سداد (${licenseInfo.graceDaysRemaining} يوم متبقي)` : 'مقفل'}
                  </span>
                )}
              </div>
              ...
              {licenseInfo && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-2.5 bg-white rounded-xl border border-slate-200 space-y-1">
                    <span className="text-[10px] text-slate-400 block">معرّف الجهاز الرقمي (Hardware ID):</span>
                    <span className="font-mono font-bold text-indigo-700 text-xs block truncate" title={licenseInfo.deviceId}>
                      {licenseInfo.deviceId}
                    </span>
                  </div>

                  <div className="p-2.5 bg-white rounded-xl border border-slate-200 space-y-1">
                    <span className="text-[10px] text-slate-400 block">تاريخ انتهاء الاشتراك السنوي:</span>
                    <span className="font-bold text-slate-700 text-xs block">
                      {new Date(licenseInfo.expiresAt).toLocaleDateString('ar-SA')}
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition active:scale-95 cursor-pointer shadow-xs"
                >
                  <KeyRound className="w-3.5 h-3.5" />
                  <span>إدخال كود التفعيل الرقمي / تجديد الاشتراك (500 ر.س)</span>
                </button>
      ...
      {/* Offline Lock Screen Overlay (For Expired or Clock-Tampered Locked states) */}
      {licenseInfo && (licenseInfo.status === 'expired_locked' || licenseInfo.status === 'tampered_locked') && (
        <OfflineLockScreen
          licenseInfo={licenseInfo}
          onActivated={() => {
            refreshLicenseState();
            addToast('تم تفعيل اشتراك قيد السنوي بنجاح! شكراً لثقتكم.', 'success');
          }}
        />
      )}

      {/* Offline License Management Modal */}
      {isLicenseModalOpen && licenseInfo && (
        <OfflineLockScreen
          licenseInfo={licenseInfo}
          onActivated={() => {
            refreshLicenseState();
            setIsLicenseModalOpen(false);
            addToast('تم تحديث وتفعيل ترخيص قيد السنوي بنجاح!', 'success');
          }}
          isModal={true}
          onCloseModal={() => setIsLicenseModalOpen(false)}
        />
      )}
    </div>
  );
